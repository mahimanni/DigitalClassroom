...
def student_home(request):
    student_obj= Students.objects.get(admin=request.user.id) #accessing current student object by admin id
    total_attendance= AttendanceReport.objects.filter(student_id= student_obj).count() #accessing count data of Attendance Report such as Total, Present, Absent
    ...
    #for fetching each subject attendance and present absent data
    ...
    return render(request,"student_template/student_home_template.html",{"total_attendance":total_attendance, "attendance_present":attendance_present, "attendance_absent":attendance_absent, "subjects":subjects, "data_name":subject_name, "data1":data_present, "data2":data_absent})
